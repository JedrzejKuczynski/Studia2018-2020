...
int main(int argc, char* argv[]) {
    ...
    printf("ZACZALEM TESTY FUNKCYJNEGO DYNAMIC\n");

    FILE *results = fopen("Testy/sito_fun_dynamic.txt", "a");
            ...
            for(int par = 0; par < 10; par++) {
                ...
                for(int it = 0; it < 3; it++) {
                    ...
                    #pragma omp parallel num_threads(num_procs[np]) private(i, j) shared(primes_to_root, primes_in_range)
                    {
                        ...
                        #pragma omp for schedule(dynamic, params[par])  // Trza ustalić testowane przedziały
